...
class ConservativeAgent:
    ...
    def strategy_score(self, step_idx: int, max_steps: int, task_key: str) -> float:
        return 0.6 if self.strategy_name(step_idx, max_steps, task_key) == "opportunistic" else 0.0
    ...
    def act(self, round_state: dict[str, float], step_idx: int, max_steps: int, task_key: str) -> float:
        ...
        if task_key == "hard" and step_idx >= max_steps // 2:
            intensity = 0.42 + (0.34 * scarcity) + (0.16 if resource_value >= 65 else 0.0)
        elif resource_value < 35:
            intensity = 0.18 + (0.10 * scarcity)
        elif resource_value < 65:
            intensity = 0.48 + (0.08 * scarcity)
        else:
            intensity = 0.68 + (0.12 * scarcity)
